...
await reply(`Berhasil menghapus ${anu.length} pribadi chat`)
break
// Case Kebawah ( MyMans APIs & Hexagon )
case 'on':
if (!mek.key.fromMe && !isOwner) return 
offline = false
...
break
	...
	case 'ytsearch':
			...
			break
// Set Reply ( MyMans APIs & Hexagonz )
case 'setreply':
case 'setfake':
if (!mek.key.fromMe && !isOwner) return
...
break
// Set Fake ( MyMans APIs & Hexagonz )
case 'setfakeimg':
if (!mek.key.fromMe && !isOwner) return
if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length == 0) {
boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
delb = await hexa.downloadMediaMessage(boij)
fs.writeFileSync(`./media/image/fake.jpeg`, delb)
fakestatus('Sukses')
} else {
reply(`Kirim gambar dengan caption ${prefix}sethumb`)
}
break	
// Set Thumbnail ( MyMans APIs & Hexagonz )
case 'setthumb':
if (!mek.key.fromMe && !isOwner) return
if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length == 0) {
boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
delb = await hexa.downloadMediaMessage(boij)
fs.writeFileSync(`./media/image/thumb.jpeg`, delb)
fakestatus('Sukses')
} else {
reply(`Kirim gambar dengan caption ${prefix}sethumb`)
}
