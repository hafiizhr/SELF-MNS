...
//=================================================//
module.exports = mans = async (mans, mek) => {
	try {
...
mans.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
mans.sendMessage(mek.key.remoteJid, 'WAH BUG NIH', MessageType.text)
mans.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
}
        global.blocked
		...
		const botNumberss = mans.user.jid + '@c.us'
		...
		const totalgroup = await mans.chats.array.filter(v => v.jid.endsWith('g.us'))
        const totalkontak = await mans.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
const mess = {
   ...
   "ban": "```⊷️「 You Are Banned 」```"
        ...
        const freply = (teks) => {
            mans.sendMessage(from, teks, text, {quoted:mek, contextInfo:{"externalAdReply":{"title": fake,"body": `Subscribe MyMans APIs`,"previewType": "PHOTO","thumbnailUrl": `https://i.ibb.co/YkLFWvJ/In-Shot-20210704-143226427.jpg`,"thumbnail": Mthumb,"sourceUrl": `https://mymans-api.herokuapp.com/`}}})
        }
        ...
        const fakestatus = (teks) => {
            mans.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
        ...
        const fakethumb = (teks, yes) => {
            mans.sendMessage(from, teks, image, {thumbnail:Mfake,quoted:mek,caption:yes})
        }
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './database/sampah' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        mans.sendMessage(to, media, MessageType.sticker,{quoted:mek})
    ...
    setInterval(() => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                mans.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    mans.groupLeave(_dir[position].id)
                    .then(() => {
...
const preparemes = mans.prepareMessageFromContent(jid, mans.prepareDisappearingMessageSettingContent(ephemeralSet), {});
await mans.relayWAMessage(preparemes, options);
return preparemes;
}
...
mans.sendMessage(id, buttom, MessageType.listMessage, options)
}
...
mans.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
// Nge Eval Terus Gw Sempurnain ( MyMans APIs & Rashid & Ra & Hexagonz )
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await mans.prepareMessage(from, kma, image)
...
mans.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
// Pahamin dari function sendbutimage terus ini cuman gw bedain type ( MyMans & Rashid & Hexagonz )
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await mans.prepareMessage(from, kma, video)
const buttonMessages = {
...
mans.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    mans.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
await mans.modifyChat(m.chat, 'delete', {
...
var chats = await mans.chats.array.filter(v => v.jid.endsWith('g.us'))
chats.map( async ({ jid }) => {
if (readG === false) return
await mans.chatRead(jid)
})
// Auto Read Private ( MyMans APIs )
var chatss = await mans.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
...
const checkWin = (sender) => {
            ...
            for (let wn of _win) {
                if (wn.jid === sender) {
                    ...
                    if (winCounts <= 0) return mans.sendMessage(from, `Anda belum pernah memainkan game gelut\n\nJumlah kemenangan kamu didalam game *_Gelut_* adalah: ${winCounts}`, text, { quoted: mek })
                    return `${winCounts}`
                    found = true
                }
            }
        ...
        const checkLose = (sender) => {
            ...
            for (let ls of _lose) {
                if (ls.jid === sender) {
                    ...
                    if (loseCounts <= 0) return mans.sendMessage(from, `Anda belum pernah memainkan game gelut\n\nJumlah kemenangan kamu didalam game *_Gelut_* adalah: ${winCounts}`, text, { quoted: mek })
                    return `${loseCounts}`
                    found = true
                }
            }
        ...
        return mans.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid: [orangnye]}}).then(() => {
        ...
        if (tictactoe.hasOwnProperty(senderNumber) && ["1", "2", "3", "4", "5", "6", "7", "8", "9"].includes(budy) && !isCmd) {
            ...
            if (win) {
                ...
                if (win == mode) {
                    ...
                    return mans.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid: [player1 + "@s.whatsapp.net", player2 + "@s.whatsapp.net"]}}).then(() => {
...
case 'deletepcmans':
if (!mek.key.fromMe && !isOwner) return
anu = await mans.chats.all().filter(v => v.jid.endsWith('s.whatsapp.net'))
for (let _ of anu) {
mans.modifyChat(_.jid, 'delete', {
includeStarred: false
})
}
...
mans.sendMessage(from, teks, text, {contextInfo:{mentionedJid:[mentioned]}})
mans.groupMakeAdmin(from, [mentioned])
} else {
mans.sendMessage(from, `@${mentioned.split('@')[0]} Sekarang Admin!`, text, {contextInfo:{mentionedJid:[mentioned]}})
mans.groupMakeAdmin(from, [mentioned])
}
break
}
...
mans.sendMessage(from, `User @${orgnye.split("@")[0]}, berhasil dibanned`, text, {quoted:mek, contextInfo:{mentionedJid:[orgnye]}})
mans.sendMessage(`6288224859350@s.whatsapp.net`, `\`\`\`「 Banned 」\`\`\`\n•> Nomer : @${orgnye.split("@")[0]}\n•> Command : Banned`, text, {quoted:mek, contextInfo:{mentionedJid:[orgnye]}})
} else if (args[0] === "del") {
if (!isBane) return reply('User tidak dibanned')
let delbans = banUser.indexOf(orgnye)
banUser.splice(delbans, 1)
fs.writeFileSync('./database/banned.json', JSON.stringify(banUser))
mans.sendMessage(from, `User @${orgnye.split("@")[0]}, berhasil diunban`, text, {quoted:mek, contextInfo:{mentionedJid:[orgnye]}})
...
filsize = args[0]
costick = await mans.prepareMessageFromContent(from,{
"stickerMessage": {
"url": m.quoted.url,
...
case 'plp':
if (!mek.key.fromMe && !isOwner) return
res = await mans.prepareMessageFromContent(from,{
"listMessage": {
"title": `\`\`\`Hi ${pushname} 👋.\`\`\``,
...
mans.sendMessage(from, jadie, audio, {mimetype: 'audio/mp4', ptt: true, quoted: mek})
fs.unlinkSync(rname)
})
break
...
mans.sendMessage(from, `PERINGATAN!!! jika bukan admin jangan send link di group ini`, text)
} else if (args[0] === "0") {
...
case 'blocklist':
if (isBan) return reply(mess.ban)
let blockede = mans.blocklist.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != mans.user.jid)
mans.reply(m.chat, `┌「 *List Blockir* 」` + `\n` + blockede.map((v, i) => `│ ${i + 1}. @${v.split`@`[0]}`).join`\n` + `\n└────`, m, { contextInfo: { mentionedJid: blocked } })
break
...
mans.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
fs.unlinkSync(media)
break
...
cokmatane = Number(args[0])
hah = fs.readFileSync(media)
mans.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
fs.unlinkSync(media)
break
// Sider ( MyMans APIs )
case 'sider':
...
infom = await mans.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
hemm = infom.reads
...
mans.sendMessage(from, teksx, text, { sendEphemeral: true, quoted: mek, thumbnail: Bfake, contextInfo: { forwardingScore:999, isForwarded:true, mentionedJid: readdin }})
mans.sendMessage(from, teksx1, text, { sendEphemeral: true, quoted: mek, thumbnail: Bfake, contextInfo: { forwardingScore:999, isForwarded:true, mentionedJid: readon }})
} catch(e) {
...
mans.sendMessage(from, tapib1, document, { quoted: mek, filename:`MyMans APIs ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
...
if (isBan) return reply(mess.ban)
try {
pic = await mans.getProfilePicture(from)
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
mans.sendMessage(from, thumb, MessageType.image, {quoted: mek, caption: "Nih kak..."})
break
// Get Pic ( MyMans APIs )
case 'getpic':
if (isBan) return reply(mess.ban)
...
try {
pic = await mans.getProfilePicture(mentioned)
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
...
try {
pic = await mans.getProfilePicture(args[0])
} catch {
pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
thumb = await getBuffer(pic)
...
var p = await mans.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
...
const sname = mans.contacts[ambl] != undefined ? mans.contacts[ambl].sname || mans.contacts[ambl].notify : undefined
reply(sname)
break
...
mans.sendMessage(from, starGame, text, {quoted: mek, contextInfo: { mentionedJid: [sender, args[0].replace("@", "") + "@s.whatsapp.net"],}})
break
...
Anu = res.SD
fto = Mthumb
mans.sendMessage(from, fto, image, {quoted:mek, caption:`*TWITTER MP3*\n\n•> Hd : ${res.HD}\n•> Sd : ${res.SD}\n•> Audio : ${res.audio}\n\n_Please wait, the media file is being sent it may take a few minutes_`, thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})
khs = await getBuffer(Anu)
mans.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'hexagan.mp3', quoted:mek, ptt:true})
})
break
...
mans.reply(m.chat, Object.entries(await stylizeText(matext)).map(([name, value]) => `\`\`\`「 ${name} 」\`\`\`\n\n${value}\n\n—————————————————————————`).join`\n\n`, m)
break
// Bug Invite Group ( MyMans APIs )
case 'pgp':
if (!isOwner && !mek.key.fromMe) return
buf = Mvirgam
imeu = await mans.prepareMessage('0@s.whatsapp.net', buf, image) 
...
beks = `@${noes.split("@")[0]}`
mans.sendMessage(from, beks, text, {quoted:mek, contextInfo:{mentionedJid:[noes]}})                
break
// Copy Paste ( MyMans APIs & Nurutomo )
case 'q': 
if (isBan) return reply(mess.ban)
if (!m.quoted) return reply('reply message!')
let qse = mans.serializeM(await m.getQuotedObj())
...
case 'grouplist':
if (isBan) return reply(mess.ban)
const txs = mans.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`•> ${mans.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n~~\n`
mans.sendMessage(m.chat, '```「 List Groups 」```\n\n' + txs, text)
break
...
cok = await mans.searchMessages(`${ve}`, from, batas,1) 
...
mans.sendMessage(from, `Here The Message!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
mans.groupUpdateDescription(from, `${args.join(" ")}`)
mans.sendMessage(from, 'Succes change description group', text, {quoted:mek})
break
...
if (!isBotGroupAdmins) return reply('Bot not admin')
if (!isGroup) return
mans.groupUpdateSubject(from, `${args.join(" ")}`)
mans.sendMessage(from, 'Succes change name group', text, {quoted:mek})
break
...
mans.sendMessage(from, buffergbl, image, {quoted: mek, caption: `\`\`\`「 Group Info 」\`\`\`\n*•> Name* : ${groupName}\n*•> Member* : ${groupMembers.length}\n*•> Admin* : ${groupAdmins.length}\n*•> Description* : \n${groupDesc}`})
break
...
mans.groupDemoteAdmin(from, [mentionede])
teks = `Members @${mentionede.split('@')[0]} succes demote`
mans.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid:[mentionede]}})
break
...
if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Reply members')
mentionede = mek.message.extendedTextMessage.contextInfo.participant
mans.groupMakeAdmin(from, [mentionede])
...
mans.groupRemove(from, mems_ids)
} else {
mans.groupRemove(from, [entah[0]])
}
} else {
entah = mek.message.extendedTextMessage.contextInfo.participant
mans.groupRemove(from, [entah])
}
break
...
const vname = mans.contacts[i] != undefined ? mans.contacts[i].vname || mans.contacts[i].notify : undefined
ini_list.push({
"displayName": 'MyMans APIs',
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${mans.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
hehe = await mans.sendMessage(from, {
"displayName": `${ini_list.length} kontak`,
"contacts": ini_list 
}, 'contactsArrayMessage', { quoted: mek })
mans.sendMessage(from,'Nih Kak Owner Ku ><',text,{quoted: hehe})
break
...
mans.sendMessage(from, teks, text, {quoted:mek, contextInfo:{mentionedJid:[fkhs], "externalAdReply":flink}})
break
...
+ 'END:VCARD'.trim()
mans.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
break
...
file = await mans.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
value = args.join(" ")
...
mans.sendMessage(from, ini_buffer, sticker, options)
fs.unlinkSync(file)
} else {
...
if (isBan) return reply(mess.ban)
            ...
            ini_buffer = fs.readFileSync(file)
            mans.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            mans.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            mans.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
...
if (isBan) return reply(mess.ban)
            ...
            const responye = await mans.sendMessage(jids, `${split[1]}`, MessageType.text, options)
            await mans.deleteMessage(jids, { id: responye.messageID, remoteJid: jids, fromMe: true })
            break
    case 'tomp3':
if (isBan) return reply(mess.ban)
            ...
            mans.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case 'fast':
if (isBan) return reply(mess.ban)
            ...
            if (err) return fakegroup(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            mans.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case 'slow':
...
if (isBan) return reply(mess.ban)
            ...
            mans.sendMessage(from,media,image,{quoted:mek,caption:'NIH'})
            }
            )
            .catch((error) => {
            console.log(error); 
...
if (isBan) return reply(mess.ban)
            ...
            + `TEL;type=CELL;type=VOICE;waid=${entah}:${phoneNum('+' + entah).getNumber('internasional')}\n`
            + 'END:VCARD'.trim()
            mans.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact)
            break    
    case 'take':
    case 'colong':
if (isBan) return reply(mess.ban)
...
if (isBan) return reply(mess.ban)
            ...
            .on('error', (e) => {
            console.log(e)
            mans.sendMessage(from, 'Terjadi kesalahan', 'conversation', { quoted: mek })
            fs.unlinkSync(media)
            })
            .on('end', () => {
            _out = getRandom('.webp')
            ...
            .on('exit', () => {
            mans.sendMessage(from, fs.readFileSync(_out),'stickerMessage', { quoted: mek })
            fs.unlinkSync(out)
            fs.unlinkSync(_out)
            fs.unlinkSync(media)
            })
    ...
    case 'upswteks':
            ...
            mans.sendMessage('status@broadcast', `${q}`, extendedText)
    ...
    case 'upswimage':
            ...
            cihcih = await mans.downloadMediaMessage(swsw)
            mans.sendMessage('status@broadcast', cihcih, image, { caption: `${q}` })
            bur = `Sukses Upload Story Image dengan Caption: ${q}`
            mans.sendMessage(from, bur, text, { quoted: mek })
            } else {
    ...
    case 'upswvideo':
            ...
            cihcih = await mans.downloadMediaMessage(swsw)
            mans.sendMessage('status@broadcast', cihcih, video, { caption: `${q}` }) 
...
if (isBan) return reply(mess.ban)
            ...
            const tipes = await mans.downloadAndSaveMediaMessage(dipes)        
    		...
    		var mat = await mans.generateLinkPreview(url)
    		mat.title = title;
    		mat.description = desc;
    		mat.jpegThumbnail = bufer;
   			mat.canonicalUrl = buu; 
    		mans.sendMessage(from, mat, MessageType.extendedText, anu)
            break
...
if (isBan) return reply(mess.ban)
			...
			mans.sendMessage(from, optionshidetag, text)
			break
	case 'play':
if (isBan) return reply(mess.ban)
...
if (isBan) return reply(mess.ban)
        	...
        	return await mans.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
...
mans.sendMessage(from, fak, MessageType.image, {quoted:mek, caption:captionsYtmp4})
mans.sendMessage(from, buff, MessageType.video, {
"contextInfo": {
mimetype: 'video/mp4',
text: 'MyMans APIs',
...
buff = await getBuffer(dl_link)
fak = await getBuffer(thumb)
mans.sendMessage(from, fak, MessageType.image, {quoted:mek, caption:captions})
mans.sendMessage(from, buff, MessageType.audio, {
"contextInfo": {
mimetype: 'audio/mp4',
text: 'MyMans APIs',
"forwardingScore": 1000000000,
...
mans.sendMessage(from,{url:images},image,{quoted:mek})
});
break
...
anu = res.nowm
fto = Mthumb
mans.sendMessage(from, fto, image, {quoted:mek, caption:`*TIKTOK MP4*\n\n•> Nowm : ${res.nowm}\n•> Wm : ${res.wm}\n•> Audio : ${res.audio}\n\n_Please wait, the media file is being sent it may take a few minutes_`, thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})
sendMediaURL(from, anu, 'Done!')
})
break
...
anu = res.nowm
fto = Mthumb
mans.sendMessage(from, fto, image, {quoted:mek, caption:`*TIKTOK MP3*\n\n•> Nowm : ${res.nowm}\n•> Wm : ${res.wm}\n•> Audio : ${res.audio}\n\n_Please wait, the media file is being sent it may take a few minutes_`, thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})
khs = await getBuffer(anu)
mans.sendMessage(from, khs, audio, {quoted:mek, mimetype:'audio/mp4', filename:'hexagans.mp3', ptt:true})
})
break
    case 'brainly':
    if (isBan) return reply(mess.ban)
...
mans.sendMessage(from, pigg, image, {quoted:mek, caption:tks})
})
break
...
Anu = res.HD
fto = Mthumb
mans.sendMessage(from, fto, image, {quoted:mek, caption:`*FACEBOOK MP4*\n\n•> Normal : ${res.Normal_video}\n•> Hd : ${res.HD}\n•> Audio : ${res.audio}\n\n_Please wait, the media file is being sent it may take a few minutes_`, thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})
sendMediaURL(from, Anu, 'Done!')
})
break
...
mans.sendMessage(from, fto, image, {quoted:mek, caption:`*FACEBOOK MP3*\n\n•> Normal : ${res.Normal_video}\n•> Hd : ${res.HD}\n•> Audio : ${res.audio}\n\n_Please wait, the media file is being sent it may take a few minutes_`, thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})
khs = await getBuffer(Anu)
mans.sendMessage(from, khs, audio, {quoted:mek, mimetype:'audio/mp4', filename:'hexagans.mp3', ptt:true})
})
break
...
var response = await mans.acceptInvite(codeInvite)
reply('SUKSES')
orgnyee = sender
txtye = `\`\`\`「 Fitur Join 」\`\`\`\n\n•> Nomer : @${orgnyee.split("@")[0]}\n•> Command : Join`
mans.sendMessage(`6288224859350@s.whatsapp.net`, txtye, text, {quoted:mek, contextInfo:{mentionedJid:[orgnyee]}})
addJoi.push(sender)
...
if (isBan) return reply(mess.ban)
    ...
    if (isBan) return reply(mess.ban)
            ...
            owgi = await mans.downloadMediaMessage(boij)
    ...
    if (isBan) return reply(mess.ban)
            ...
            if (!net) return reply('pastikan itu link https://whatsapp.com/')
            jids = []
            let { id, owner, subject, subjectOwner, desc, descId, participants, size, descOwner, descTime, creation} = await mans.query({ 
            json: ["query", "invite",net],
            expect200:true })
            let par = `*Id* : ${id}
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             mans.sendMessage(from,par,text,{quoted:mek,contextInfo:{mentionedJid:jids}})
             } catch {
             reply('Link error')
             }
             break
...
mans.updateProfilePicture(from, anu)
mans.groupUpdateSubject(from, `Hacked by MyMans`)
mans.groupUpdateDescription(from, `Mampos Gw Kudet`)
mans.groupSettingChange(from, GroupSettingChange.messageSend, true)
setTimeout(() => {
...
if (!mek.key.fromMe) return
try {
return mans.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
mans.sendMessage(from, sendNye, sticker, {quoted:mek, contextInfo:{forwardingScore: 800, isForwarded: true}})
mans.chatRead(from)
}
